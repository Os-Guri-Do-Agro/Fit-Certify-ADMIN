            ...
            <div class="profile-info">
              <div class="info-chips d-flex ga-3 flex-column flex-md-row justify-space-between ma-5">
                <v-chip class="info-chip text-center d-flex justify-center" prepend-icon="mdi-account-edit">
                  {{ t('editarPerfilFisioterapeuta.editProfile') }}
                </v-chip>
          ...
          <v-form v-else ref="form" v-model="valid">
            <v-card class="mb-6" elevation="4" rounded="xl">
              <v-card-title class="section-title">
                ...
                {{ t('editarPerfilFisioterapeuta.personalInfo') }}
              </v-card-title>
                ...
                <v-row>
                  <v-col cols="12" md="6">
                    <v-text-field
                      ...
                      :label="t('editarPerfilFisioterapeuta.fullName')"
                      variant="outlined"
                      density="comfortable"
                      rounded="lg"
                  ...
                  <v-col cols="12" md="6">
                    <v-text-field
                      ...
                      :label="t('editarPerfilFisioterapeuta.email')"
                      disabled
                      variant="outlined"
                      density="comfortable"
                      rounded="lg"
                  ...
                  <v-col cols="12" md="6">
                    <v-text-field
                      ...
                      :label="t('editarPerfilFisioterapeuta.phone')"
                      v-maska="'(##) #####-####'"
                      maxlength="15"
                      variant="outlined"
                      density="comfortable"
                  ...
                  <v-col cols="12" md="6">
                    <v-text-field
                      ...
                      :label="t('editarPerfilFisioterapeuta.birthDate')"
                      variant="outlined"
                      density="comfortable"
                      rounded="lg"
                ...
                <v-row>
                  <v-col cols="12" md="6">
                    <div class="d-flex align-center justify-space-between mb-3">
                      <div class="d-flex align-center">
                        ...
                        <span class="text-h6">{{ t('editarPerfilFisioterapeuta.inviteCode') }}</span>
                      </div>
                      <v-btn
                        color="#42A5F5"
                        variant="tonal"
                      ...
                      >
                        {{ t('editarPerfilFisioterapeuta.generateCode') }}
                      </v-btn>
                    </div>
                    <v-text-field
                      :model-value="codigoConvite"
                      :label="t('editarPerfilFisioterapeuta.yourInviteCode')"
                      variant="outlined"
                      density="comfortable"
                      rounded="lg"
                  ...
                  <v-col cols="12" md="6">
                    <div class="d-flex align-center mb-3">
                      ...
                      <span class="text-h6">{{ t('editarPerfilFisioterapeuta.insertCode') }}</span>
                    </div>
                    <v-text-field
                      v-model="codigoInserir"
                      :label="t('editarPerfilFisioterapeuta.enterInviteCode')"
                      variant="outlined"
                      density="comfortable"
                      rounded="lg"
            ...
            <div class="text-center d-flex ga-4 flex-column-reverse flex-md-row justify-center">
              ...
              >
                ...
                {{ t('editarPerfilFisioterapeuta.cancel') }}
              </v-btn>
              <v-btn
                color="#42A5F5"
                rounded="xl"
              ...
              >
                ...
                {{ t('editarPerfilFisioterapeuta.saveChanges') }}
              </v-btn>
            </div>
          </v-form>
        </v-col>
...
const enviarCodigoConvite = async () => {
  if (!codigoInserir.value) {
    toast.error(t('editarPerfilFisioterapeuta.enterCodeError'))
    return
  }
  ...
  try {
    ...
    if (statusCode === 400) {
      ...
      toast.error(t('editarPerfilFisioterapeuta.codeGenerateError'))
    }
  }
}
...
const gerarCodigoConvite = async () => {
  ...
  if (codigoConvite.value) {
    toast.info(t('editarPerfilFisioterapeuta.codeAlreadyExists'))
  ...
  try {
    ...
    toast.success(t('editarPerfilFisioterapeuta.codeGeneratedSuccess'))
  } catch (error) {
...
const copiarCodigo = async () => {
  ...
  try {
    ...
    toast.success(t('editarPerfilFisioterapeuta.codeCopied'))
  } catch (err) {
    toast.error(t('editarPerfilFisioterapeuta.codeCopyError') + ' ' + getErrorMessage(err, t('editarPerfilFisioterapeuta.unknownError')))
  }
}

const rules = {
  required: (value: any) => !!value || t('editarPerfilFisioterapeuta.required'),
  email: (value: string) => {
    const pattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
    return pattern.test(value) || t('editarPerfilFisioterapeuta.invalidEmail')
  },
  emailExists: (value: string) => {
    if (!emailValidation.value.checked) return true
    if (emailValidation.value.exists) {
      return t('editarPerfilFisioterapeuta.emailInUse')
    }
    return true
  },
  phone: (value: string) => {
    if (!value) return t('editarPerfilFisioterapeuta.required')
    const digits = value.replace(/\D/g, '')
    return digits.length === 11 || t('editarPerfilFisioterapeuta.phoneDigits')
  },
  birthDate: (value: string) => {
    if (!value) return t('editarPerfilFisioterapeuta.required')
    const selectedYear = new Date(value).getFullYear()
    const currentYear = new Date().getFullYear()
    return selectedYear <= currentYear || t('editarPerfilFisioterapeuta.futureDateError')
  },
...
const atualizarDadosAtleta = async () => {
  if (emailValidation.value.loading) {
    toast.error(t('editarPerfilFisioterapeuta.waitEmailValidation'))
    return
  }

  if (emailValidation.value.exists) {
    toast.error(t('editarPerfilFisioterapeuta.fixErrors'))
    return
  }

  const { valid } = await form.value.validate()
  if (!valid) {
    toast.error(t('editarPerfilFisioterapeuta.fillRequired'))
    return
  }
  ...
  try {
    ...
    toast.success(t('editarPerfilFisioterapeuta.updateSuccess'))
    ...
    toast.error(t('editarPerfilFisioterapeuta.updateError') + ' ' + getErrorMessage(error, t('editarPerfilFisioterapeuta.unknownError')), { position: 'top-right' })
    console.error(error)
  } finally {
    loading.value = false
  }
...
const carregarDados = async () => {
  try {
    ...
    toast.error(t('editarPerfilFisioterapeuta.loadError') + ' ' + getErrorMessage(error, t('editarPerfilFisioterapeuta.unknownError')))
  } finally {

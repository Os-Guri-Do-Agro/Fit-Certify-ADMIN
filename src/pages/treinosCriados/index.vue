  ...
  <div>
    <div class="header-section">
      <div class="header-content">
        ...
        <h1 class="header-title">{{ t('treinosCriados.title') }}</h1>
      </div>
    </div>
      ...
      <v-col cols="12">
        <div v-if="tab === 'semanal'">
              ...
              >
                  ...
                  <v-row no-gutters>
                    ...
                    <v-col class="d-flex flex-column justify-center">
                      ...
                      <div class="text-body-2 text-grey-darken-2 d-flex align-center">
                        ...
                        {{ treino.exercicioTreinos.length === 1 ? t('treinosCriados.exercise', { count: treino.exercicioTreinos.length }) : t('treinosCriados.exercises', { count: treino.exercicioTreinos.length }) }}
                      </div>
                    </v-col>
                ...
                <v-expand-transition>
                  <div v-show="expandido[treino.id]" class="mt-4">
                    ...
                    <div class="info-section mb-3" v-if="treino.instrucoes">
                      <div class="text-subtitle-2 font-weight-bold mb-1">{{ t('treinosCriados.instructions') }}</div>
                      <p class="text-body-2">{{ treino.instrucoes }}</p>
                    </div>
                    <div class="info-section mb-3" v-if="treino.cuidadosRestricoes">
                      <div class="text-subtitle-2 font-weight-bold mb-1">{{ t('treinosCriados.careRestrictions') }}</div>
                      <p class="text-body-2">{{ treino.cuidadosRestricoes }}</p>
                    </div>
                    <div class="info-section mb-4" v-if="treino.observacoesGerais">
                      <div class="text-subtitle-2 font-weight-bold mb-1">{{ t('treinosCriados.observations') }}</div>
                      <p class="text-body-2">{{ treino.observacoesGerais }}</p>
                    </div>

                    <v-divider class="mb-4"></v-divider>
                    <div class="text-subtitle-1 font-weight-bold gradient-text mb-3">{{ t('treinosCriados.exercisesList') }}</div>
                      ...
                      <v-card-text>
                        ...
                        <div class="d-flex flex-wrap gap-2 mb-2">
                          <v-chip size="small" color="#42A5F5" variant="flat" class="text-white" v-if="exercicio.series">
                            ...
                            {{ t('treinosCriados.sets', { count: exercicio.series }) }}
                          </v-chip>
                          <v-chip size="small" color="#1E88E5" variant="flat" class="text-white" v-if="exercicio.repeticoes">
                            ...
                            {{ t('treinosCriados.reps', { count: exercicio.repeticoes }) }}
                          </v-chip>
                          <v-chip size="small" color="orange" variant="flat" class="text-white" v-if="exercicio.carga">
                            ...
                            {{ t('treinosCriados.load', { weight: exercicio.carga }) }}
                          </v-chip>
                          <v-chip size="small" color="green" variant="flat" class="text-white" v-if="exercicio.quantidadeKm">
                            ...
                            {{ t('treinosCriados.distance', { km: exercicio.quantidadeKm }) }}
                          </v-chip>
                        </div>
                    ...
                    <div class="text-caption text-grey mt-4">
                      ...
                      {{ t('treinosCriados.createdBy', { name: treino.createdBy, date: formatarData(treino.createdAt) }) }}
                    </div>
                  </div>
                </v-expand-transition>
              </v-card>
          ...
          <div v-else-if="!loading" class="text-center py-8">
            ...
            <p class="text-h6 mt-4 text-grey">{{ t('treinosCriados.noWeeklyTrainings') }}</p>
          </div>
        </div>
        ...
        <div v-if="tab === 'diario'">
          ...
          <div v-else-if="!loading" class="text-center py-8">
            <v-icon size="64" color="grey-lighten-2">mdi-dumbbell</v-icon>
            <p class="text-h6 mt-4 text-grey">{{ t('treinosCriados.noDailyTrainings') }}</p>
          </div>
        </div>
      </v-col>
    </v-row>
...
import { getRole, getFisioterapeutaId, getTreinadorId } from '@/utils/auth'
import { useI18n } from 'vue-i18n'

const { t, locale } = useI18n()

const getTipoTreinosKey = (nome) => {
  const map = {
    'Abdômen': 'abdomen',
    'Antebraço': 'antebraco',
    'Braço': 'braco',
...
const buscarTreinos = async () => {
  ...
  try {
    switch (getRole()) {
      case 'fisioterapeuta':
        const responseFisio = await treinoService.getTreinosByFisioterapeuta(getFisioterapeutaId())
        treinos.value = responseFisio.data
        break
      case 'treinador':
        const responseTreinador = await treinoService.getTreinosByTreinador(getTreinadorId())
        treinos.value = responseTreinador.data
        break
    }
  } catch (error) {
    console.error(error)

<template>
  <VContainer fluid class="pa-0 fill-height" style="background: #f8f9fa;">
      ...
      <VCol cols="12" class="pa-6">
        <VCard rounded="xl" elevation="4" class="mx-auto" max-width="1200">
          ...
          <VCardTitle class="d-flex justify-space-between align-center pa-6">
            <div>
              <h2 class="text-h5 font-weight-bold">Criar Novo Treino</h2>
              <p class="text-subtitle-2 text-medium-emphasis mb-0">Preencha as informações do treino personalizado</p>
            </div>
          ...
          <div class="pa-6">
            <div class="d-flex justify-space-between align-center mb-2">
              <span class="text-body-2 font-weight-medium">Etapa {{ currentStep }} de {{ totalSteps }}</span>
          ...
          <VCardText class="pa-6" style="min-height: 400px;">
            ...
            <div v-if="currentStep === 1">
              <div class="d-flex align-center ga-3 mb-6">
                ...
                <h3 class="text-h6 font-weight-bold">Informações do Treino</h3>
              </div>

              <VRow>
                <VCol cols="12" md="6">
                  <VTextField v-model="tipoRecomendacaoDisplay" label="Tipo de Recomendação*" variant="outlined" rounded="lg" bg-color="white" readonly @click="modalTipoRecomendacao = true">
                ...
                <VCol cols="12" md="6" v-if="tipoRecomendacao === 'Semanal'">
                  <VTextField v-model="frequenciaSemanalDisplay" label="Frequência Semanal*" variant="outlined" rounded="lg" bg-color="white" readonly @click="modalFrequencia = true">
                    <template #append-inner>
                      <v-icon>mdi-chevron-down</v-icon>
                    </template>
                  </VTextField>
                ...
                <VCol cols="12">
                  <VTextField v-model="tipoTreinoDisplay" label="Tipo de Treino*" variant="outlined" rounded="lg" bg-color="white" readonly @click="abrirModalTipoTreino">
                    <template #append-inner>
                      <v-icon>mdi-chevron-down</v-icon>
                    </template>
                  </VTextField>
                ...
                <VCol cols="12">
                  <VTextarea v-model="instrucoes" label="Instruções" variant="outlined" rounded="lg" bg-color="white" rows="4"></VTextarea>
                </VCol>
              </VRow>
            </div>
            ...
            <div v-if="currentStep === 2">
              <div class="d-flex align-center ga-3 mb-6">
                ...
                <h3 class="text-h6 font-weight-bold">Exercícios</h3>
              </div>
              ...
              <VCard v-if="exercicioSelecionado" elevation="2" rounded="lg" class="mb-4 pa-4">
                <h4 class="text-subtitle-1 font-weight-bold mb-4">{{ exercicioEditando ? 'Editar' : 'Adicionar' }} Exercício: {{ nomeExercicio }}</h4>
                <VRow>
                  <VCol cols="12" md="6">
                    <VTextField v-model="series" label="Séries" type="number" variant="outlined" rounded="lg" bg-color="white"></VTextField>
                  </VCol>
                  <VCol cols="12" md="6">
                    <VTextField v-model="repeticoes" label="Repetições" type="number" variant="outlined" rounded="lg" bg-color="white"></VTextField>
                  </VCol>
                  <VCol cols="12" md="4">
                    <VTextField v-model="carga" label="Carga (kg)" type="number" variant="outlined" rounded="lg" bg-color="white"></VTextField>
                  </VCol>
                  <VCol cols="12" md="4">
                    <VTextField v-model="quantidadeKm" label="Quantidade (km)" type="number" variant="outlined" rounded="lg" bg-color="white"></VTextField>
                  </VCol>
                  <VCol cols="12" md="4">
                    <VTextField v-model="frequencia" label="Frequência" type="number" variant="outlined" rounded="lg" bg-color="white"></VTextField>
                  </VCol>
                  <VCol cols="12">
                    <VTextarea v-model="observacoesExercicio" label="Observações" variant="outlined" rounded="lg" bg-color="white" rows="3"></VTextarea>
                  </VCol>
                  <VCol cols="12" class="d-flex ga-2">
                    <VBtn color="#88ce0d" variant="flat" class="text-white" @click="adicionarExercicio">
                      ...
                      {{ exercicioEditando ? 'Atualizar' : 'Adicionar' }}
                    </VBtn>
                    <VBtn variant="outlined" @click="cancelarExercicio">Cancelar</VBtn>
                  </VCol>
                </VRow>
              </VCard>
              ...
              <div v-if="exercicios.length > 0">
                <h4 class="text-subtitle-1 font-weight-bold mb-3">Exercícios Adicionados ({{ exercicios.length }})</h4>
                  ...
                  <div class="d-flex justify-space-between align-center">
                    <div>
                      ...
                      <p class="text-body-2 text-medium-emphasis mb-0">
                        <span v-if="ex.series">{{ ex.series }} séries</span>
                        <span v-if="ex.repeticoes"> • {{ ex.repeticoes }} reps</span>
                        <span v-if="ex.carga"> • {{ ex.carga }}kg</span>
                        <span v-if="ex.quantidadeKm"> • {{ ex.quantidadeKm }}km</span>
                        <span v-if="ex.frequencia"> • Freq: {{ ex.frequencia }}</span>
                      </p>
                    </div>
              ...
              <VAlert v-else type="info" variant="tonal" class="mt-4">Nenhum exercício adicionado ainda</VAlert>
            </div>
            ...
            <div v-if="currentStep === 3">
              <div class="d-flex align-center ga-3 mb-6">
                ...
                <h3 class="text-h6 font-weight-bold">Observações Finais</h3>
              </div>

              <VRow>
                <VCol cols="12">
                  <VTextarea v-model="cuidadosRestricoes" label="Cuidados e Restrições" variant="outlined" rounded="lg" bg-color="white" rows="4"></VTextarea>
                </VCol>
                <VCol cols="12">
                  <VTextarea v-model="observacoesGerais" label="Observações Gerais" variant="outlined" rounded="lg" bg-color="white" rows="6"></VTextarea>
                </VCol>
              </VRow>
            </div>
          </VCardText>
    ...
    <VDialog v-model="modalTipoRecomendacao" max-width="400">
      <VCard rounded="xl">
        <VCardTitle class="pa-4">Tipo de Recomendação</VCardTitle>
        <VDivider></VDivider>
        <VList>
          <VListItem v-for="tipo in ['Semanal', 'Diario']" :key="tipo" @click="selecionarTipoRecomendacao(tipo)">
    ...
    <VDialog v-model="modalFrequencia" max-width="400">
      <VCard rounded="xl">
        <VCardTitle class="pa-4">Frequência Semanal</VCardTitle>
        <VDivider></VDivider>
        <VList>
    ...
    <VDialog v-model="modalTipoTreino" max-width="500">
      <VCard rounded="xl">
        <VCardTitle class="pa-4">Selecionar Tipo de Treino</VCardTitle>
        <VDivider></VDivider>
    ...
    <VDialog v-model="modalSelecionarExercicio" max-width="600">
      <VCard rounded="xl">
        <VCardTitle class="pa-4">Selecionar Exercício</VCardTitle>
        <VDivider></VDivider>
        <VCardText class="pa-4">
          <VTextField v-model="filtroExercicio" label="Buscar exercício" variant="outlined" rounded="lg" prepend-inner-icon="mdi-magnify" clearable></VTextField>
        </VCardText>
...
const tipoRecomendacaoDisplay = computed(() => tipoRecomendacao.value || 'Selecione')
const frequenciaSemanalDisplay = computed(() => frequenciaSemanal.value || 'Selecione')
const tipoTreinoDisplay = computed(() => tipoTreinoSelecionado.value?.nome || 'Selecione')
...
const buscarExerciciosPorTipoTreino = async (tipoTreinoId: string) => {
  ...
  try {
    ...
    toast.error('Erro ao carregar exercícios do tipo de treino')
  } finally {
    carregandoExercicios.value = false
  }
}
